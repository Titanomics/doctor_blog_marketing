    ...
    return (
      <main className="flex-1 flex items-center justify-center">
        <div className="text-center">
          <p className="text-slate-400 text-lg">좌측에서 병원을 선택하세요</p>
    ...
    <main className="flex-1 overflow-y-auto p-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <form
          ...
          className="flex items-center gap-3 px-5 py-4 border-t border-slate-100 bg-slate-50/50"
        >
          <input
            type="text"
            value={newKeyword}
            onChange={(e) => setNewKeyword(e.target.value)}
            placeholder="새 키워드 입력 (예: 강남 리쥬란힐러)"
            className="flex-1 px-4 py-2 text-sm rounded-xl border border-slate-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-500/20 outline-none text-slate-800 placeholder:text-slate-400"
          />
          <motion.button
            whileTap={{ scale: 0.97 }}
            type="submit"
            disabled={addingKeyword || !newKeyword.trim()}
            className="px-5 py-2 bg-emerald-500 hover:bg-emerald-600 disabled:bg-slate-300 text-white text-sm font-medium rounded-xl transition-colors"
          >
            {addingKeyword ? "추가 중..." : "추가"}
          </motion.button>
        </form>
      </div>
      <AnimatePresence>
